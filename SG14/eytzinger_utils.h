...
constexpr int depth_of_complete_tree(int n)  // VERIFIED
{
    int depth = 0;
    while ((1 << depth) <= n) {
        ++depth;
...
constexpr int nodes_in_complete_tree(int depth)  // VERIFIED
{
    return (1 << depth) - 1;
}
...
constexpr int bit_reverse(int x, int bits)  // VERIFIED
{
    int result = 0;
...
static_assert(bit_reverse(1, 1) == 1);
static_assert(bit_reverse(1, 2) == 2);
static_assert(bit_reverse(1, 3) == 4);
static_assert(bit_reverse(1, 4) == 8);
static_assert(bit_reverse(2, 4) == 4);
static_assert(bit_reverse(3, 4) == 12);
static_assert(bit_reverse(4, 4) == 2);
static_assert(bit_reverse(5, 4) == 10);
static_assert(bit_reverse(6, 4) == 6);
static_assert(bit_reverse(7, 4) == 14);
static_assert(bit_reverse(8, 4) == 1);

constexpr int eytzinger_from_rank_in_complete_tree(int i, int depth)  // VERIFIED
{
...
constexpr int rank_from_eytzinger_in_complete_tree(int i, int depth)  // VERIFIED
{
    constexpr_assert(0 <= i && i < nodes_in_complete_tree(depth));
...
constexpr int rank_from_eytzinger(int i, int n)  // VERIFIED
{
...
constexpr int eytzinger_from_rank(int i, int n)  // VERIFIED
{
    constexpr_assert(0 <= i && i < n);
    if (n == 1) return 0;
    int D = depth_of_complete_tree(n);
    int C = (1 << D) - 1;  // number of nodes in the complete tree
    int CT_leaf_nodes_present = n - (C >> 1);
    int CT_leaf_nodes_absent = C - (C >> 1) - CT_leaf_nodes_present;
    int CT_leaf_nodes_leq_i = (i+2) >> 1;
    if (CT_leaf_nodes_present >= CT_leaf_nodes_leq_i) {
        return eytzinger_from_rank_in_complete_tree(i, D);
    } else {
        int j = i + i + CT_leaf_nodes_absent - n;
