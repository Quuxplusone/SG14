...
// but with the underlying container stored in no particular order.
...
class flat_set {
    ...
    static_assert(std::is_convertible<decltype(std::declval<const KeyEqual&>()(std::declval<const Key&>(), std::declval<const Key&>())), bool>::value, "");
#if defined(__cpp_lib_is_swappable)
...
// This is all one massive overload set!
    ...
    flat_set() : flat_set(KeyEqual()) {}

    explicit flat_set(KeyContainer ctr)
        : c_(static_cast<KeyContainer&&>(ctr)), key_equal_()
    {
    ...
    flat_set(KeyContainer ctr, const Alloc& a)
        : c_(flatset_detail::make_obj_using_allocator<KeyContainer>(a, static_cast<KeyContainer&&>(ctr))), key_equal_()
    {
        this->sort_and_unique_impl();
    }
    ...
    explicit flat_set(const Container& cont)
        : flat_set(std::begin(cont), std::end(cont), KeyEqual()) {}

    template<class Container,
             class = std::enable_if_t<flatset_detail::qualifies_as_range<const Container&>::value>>
    flat_set(const Container& cont, const KeyEqual& comp)
    ...
    flat_set(const Container& cont, const Alloc& a)
        : flat_set(std::begin(cont), std::end(cont), KeyEqual(), a) {}

    template<class Container, class Alloc,
             class = std::enable_if_t<flatset_detail::qualifies_as_range<const Container&>::value && std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(const Container& cont, const KeyEqual& comp, const Alloc& a)
    ...
    flat_set(sorted_unique_t s, const Container& cont)
        : flat_set(s, std::begin(cont), std::end(cont), KeyEqual()) {}

    template<class Container,
             class = std::enable_if_t<flatset_detail::qualifies_as_range<const Container&>::value>>
    flat_set(sorted_unique_t s, const Container& cont, const KeyEqual& comp)
        : flat_set(s, std::begin(cont), std::end(cont), comp) {}

    template<class Container, class Alloc,
             class = std::enable_if_t<flatset_detail::qualifies_as_range<const Container&>::value && std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(sorted_unique_t s, const Container& cont, const Alloc& a)
        : flat_set(s, std::begin(cont), std::end(cont), KeyEqual(), a) {}

    template<class Container, class Alloc,
             class = std::enable_if_t<flatset_detail::qualifies_as_range<const Container&>::value && std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(sorted_unique_t s, const Container& cont, const KeyEqual& comp, const Alloc& a)
        : flat_set(s, std::begin(cont), std::end(cont), comp, a) {}

    explicit flat_set(const KeyEqual& comp)
        : c_(), key_equal_(comp) {}

    template<class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(const KeyEqual& comp, const Alloc& a)
        : c_(flatset_detail::make_obj_using_allocator<KeyContainer>(a)), key_equal_(comp) {}
    ...
    template<class InputIterator>
    flat_set(InputIterator first, InputIterator last, const KeyEqual& comp = KeyEqual())
        : c_(first, last), key_equal_(comp)
    {
        this->sort_and_unique_impl();
    }
    ...
    template<class InputIterator, class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(InputIterator first, InputIterator last, const KeyEqual& comp, const Alloc& a)
        : c_(flatset_detail::make_obj_using_allocator<KeyContainer>(a)), key_equal_(comp)
    {
        while (first != last) {
    ...
    flat_set(InputIterator first, InputIterator last, const Alloc& a)
        : flat_set(first, last, KeyEqual(), a) {}

    template<class InputIterator>
    flat_set(sorted_unique_t, InputIterator first, InputIterator last, const KeyEqual& comp = KeyEqual())
        : c_(first, last), key_equal_(comp) {}
    ...
    flat_set(sorted_unique_t, InputIterator first, InputIterator last,
             const KeyEqual& comp, const Alloc& a)
        : c_(flatset_detail::make_obj_using_allocator<KeyContainer>(a)), key_equal_(comp)
    {
        while (first != last) {
            c_.insert(c_.end(), *first);
    ...
    flat_set(sorted_unique_t s, InputIterator first, InputIterator last, const Alloc& a)
        : flat_set(s, first, last, KeyEqual(), a) {}

    // TODO: should this be conditionally noexcept?
    template<class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(flat_set&& m, const Alloc& a)
        : c_(flatset_detail::make_obj_using_allocator<KeyContainer>(a, static_cast<KeyContainer&&>(m.c_))), key_equal_(static_cast<KeyEqual&&>(m.key_equal_)) {}

    template<class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(const flat_set& m, const Alloc& a)
        : c_(flatset_detail::make_obj_using_allocator<KeyContainer>(a, m.c_)), key_equal_(m.key_equal_) {}

    flat_set(std::initializer_list<Key>&& il, const KeyEqual& comp = KeyEqual())
        : flat_set(il, comp) {}

    template<class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(std::initializer_list<Key>&& il, const KeyEqual& comp, const Alloc& a)
        : flat_set(il, comp, a) {}

    template<class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(std::initializer_list<Key>&& il, const Alloc& a)
        : flat_set(il, KeyEqual(), a) {}

    flat_set(sorted_unique_t s, std::initializer_list<Key>&& il, const KeyEqual& comp = KeyEqual())
        : flat_set(s, il, comp) {}

    template<class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(sorted_unique_t s, std::initializer_list<Key>&& il, const KeyEqual& comp, const Alloc& a)
        : flat_set(s, il, comp, a) {}

    template<class Alloc,
             class = std::enable_if_t<std::uses_allocator<KeyContainer, Alloc>::value>>
    flat_set(sorted_unique_t s, std::initializer_list<Key>&& il, const Alloc& a)
        : flat_set(s, il, KeyEqual(), a) {}
...
// ========================================================== OTHER MEMBERS
    ...
    std::pair<iterator, bool> insert(Key&& t) {
        auto it = this->find(t);
        if (it == this->end()) {
            it = c_.emplace(c_.end(), static_cast<Key&&>(t));
            return {it, true};
        } else {
            return {it, false};
        }
    ...
    void insert(sorted_unique_t, InputIterator first, InputIterator last) {
        ...
        while (first != last) {
            ...
            for (auto it = c_.begin(); it != original_end; ++it) {
                if (key_equal_(*it, *first)) {
                    found = true;
                    break;
                }
            }
    ...
    void swap(flat_set& m)
        noexcept(flatset_detail::is_nothrow_swappable<KeyEqual>::value)
    {
        using std::swap;
        swap(key_equal_, m.key_equal_);
        swap(c_, m.c_);
    }
    ...
    KeyEqual key_eq() const { return key_equal_; }

    iterator find(const Key& t) {
        return std::find_if(c_.begin(), c_.end(), [&](auto&& elt) {
            return key_equal_(t, elt);
        });
    }
    ...
    template<class K,
             class KeyEqual_ = KeyEqual, class = typename KeyEqual_::is_transparent>
    iterator find(const K& x) {
        return std::find_if(c_.begin(), c_.end(), [&](auto&& elt) {
            return key_equal_(x, elt);
        });
    }
...
private:
    void sort_and_unique_impl() {
        auto last = c_.end();
        for (auto first = c_.begin(); first != last; ++first) {
            for (auto second = std::next(first); second != last; ) {
                if (key_equal_(*first, *second)) {
                    --last;
                    if (second != last) {
                        using std::swap;
                        swap(*second, *last);
...
template<class Key, class KeyEqual, class KeyContainer>
bool operator==(const flat_set<Key, KeyEqual, KeyContainer>& x, const flat_set<Key, KeyEqual, KeyContainer>& y)
{
...
template<class Key, class KeyEqual, class KeyContainer>
void swap(flat_set<Key, KeyEqual, KeyContainer>& x, flat_set<Key, KeyEqual, KeyContainer>& y) noexcept(noexcept(x.swap(y)))
{
    return x.swap(y);
}
...
template<class InputIterator, class KeyEqual = std::equal_to<flatset_detail::iter_value_type<InputIterator>>,
         class = std::enable_if_t<flatset_detail::qualifies_as_input_iterator<InputIterator>::value &&
                                  !flatset_detail::qualifies_as_allocator<KeyEqual>::value>>
flat_set(InputIterator, InputIterator, KeyEqual = KeyEqual())
    -> flat_set<flatset_detail::iter_value_type<InputIterator>, KeyEqual>;

template<class InputIterator, class KeyEqual, class Allocator,
         class = std::enable_if_t<flatset_detail::qualifies_as_input_iterator<InputIterator>::value &&
                                  !flatset_detail::qualifies_as_allocator<KeyEqual>::value &&
                                  flatset_detail::qualifies_as_allocator<Allocator>::value>>
flat_set(InputIterator, InputIterator, KeyEqual, Allocator)
    -> flat_set<flatset_detail::iter_value_type<InputIterator>, KeyEqual>;
